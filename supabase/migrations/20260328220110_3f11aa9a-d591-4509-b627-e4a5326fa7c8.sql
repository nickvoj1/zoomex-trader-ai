...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'market_snapshots' AND policyname = 'Authenticated users can read market snapshots'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'execution_events' AND policyname = 'Users can read own execution events'
  ) THEN
    create policy "Users can read own execution events"
      on public.execution_events for select to authenticated using (user_id = auth.uid() or user_id is null);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'position_reconciliations' AND policyname = 'Users can read own reconciliations'
  ) THEN
    create policy "Users can read own reconciliations"
      on public.position_reconciliations for select to authenticated using (user_id = auth.uid());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'trade_tca' AND policyname = 'Users can read own trade TCA'
  ) THEN
    create policy "Users can read own trade TCA"
      on public.trade_tca for select to authenticated using (user_id = auth.uid());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'research_runs' AND policyname = 'Users can read own research runs'
  ) THEN
    create policy "Users can read own research runs"
      on public.research_runs for select to authenticated using (user_id = auth.uid() or user_id is null);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'model_artifacts' AND policyname = 'Users can read own model artifacts'
  ) THEN
    create policy "Users can read own model artifacts"
      on public.model_artifacts for select to authenticated using (user_id = auth.uid() or user_id is null);
  END IF;
END $$;